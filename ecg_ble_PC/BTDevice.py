...
class BTDevice(object):
	...
	st='\x01' #command
	st=st+'\x05\xFE' # 0xFE GAP_DeviceDiscoveryCancel
	st=st+'\x00' # Datalength 
	#self.nextWriteCommand=st	
	self.ser.write(st)
	...
	st=st+'\x02' #data
	st=st+self.connHandle #conn handle   GetBY BLA!!!!
	#self.nextWriteCommand=st
	self.ser.write(st)
	...
	self.ser.write(st)
    def setUpCCCNotifications(self):
	st='\x01' #command
	st=st+'\x92\xFD'   # 0xFD88 (GATT_DiscCharsByUUID)
	st=st+'\x06'		#data length
	st = st+self.connHandle	#handle	
	st=st+'\x26\x00'	#Starting handle
	st=st+'\x01\x00'	#end handle
	self.ser.write(st)
    def activateSENSOR(self):
	#Write Command
	st = '\x01' #command
	st = st+'\x92\xFD'   #0xFD12 (ATT_WriteReq)
	st = st+'\x05'	#datalength
	st = st+self.connHandle	#handle
	st = st+'\x29\x00'		#CHANGED from \x21\x00! attribute Address
	st = st+'\x01'	#AttrValue
	self.ser.write(st)
	#01 92 FD 06 00 00 26 00 01 00 
	#01 92 FD 05 00 00 29 00 01
    def activateIRtemp(self):
	#Write Command
	st = '\x01' #command
